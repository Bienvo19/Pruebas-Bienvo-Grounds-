...
def test_menu_section(driver):
    ...
    try:
        ...
        # Hacer clic en el enlace "Menu" en el nav para navegar hasta la sección de menú
        menu_link = driver.find_element(By.CSS_SELECTOR, "a.nav-item.nav-link[href='#Menu']")
        menu_link.click()

        # Esperar un poco para asegurarse de que la sección de menú sea visible
        time.sleep(2)

        # Capturar la pantalla de la sección de menú
        screenshots_folder = os.path.join(os.getcwd(), "Screenshots")
        if not os.path.exists(screenshots_folder):
            os.makedirs(screenshots_folder)
        screenshot_filename = f"Menu_Section_{timestamp}.png"
        ...
        # Generar el reporte
        report_status = "Éxito"
        report_description = "Se capturó correctamente la imagen de la sección de menú."
        ...
        report_filename = f"Menu_Section_Test_{timestamp}.html"

        report_title = "Reporte de Prueba Automatizada: Sección de Menú"
